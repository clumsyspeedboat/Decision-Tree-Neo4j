...
# Splitting the data set into Training Set (2/3) & Testing Set (1/3) #

n = nrow(data_matrix)
split = sample(c(TRUE, FALSE), n, replace=TRUE, prob=c(2/3, 1/3))


TrainingSet = data_matrix[split, ]
TestingSet = data_matrix[!split, ]
...
tree2 <- C5.0(DEATH_EVENT~., data = TrainingSet)
plot(tree2)
...
levels2 <- levels[order(levels)]    
cm2 <- table(ordered(Prediction2,levels), ordered(TestingSet$DEATH_EVENT, levels))
cm2
###########################################
