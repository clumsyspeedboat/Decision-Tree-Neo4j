...
public class Continuous_Prob {
	
	private Attribute attribute;
	private double threshold;
	private double gini_impurity = -1;
	 ...
	 * @throws IOException
	 */
	public Continuous_Prob(Attribute attribute, Attribute target, 
		...
		for (int i = 0; i < instances.size() - 1; i++) {
			...
			if (!instanceValue.equals(instanceValue2)) {
				double curr_Gini_imp = calculateConti(attribute, target, instances, i);
				if (curr_Gini_imp - gini_impurity > 0) {
					gini_impurity = curr_Gini_imp;
					thresholdPos = i;
				}
			}
		}	
	...
	public static double calculateConti(Attribute attribute, Attribute target, 
		...
		double gini_imp = Gini_Impurity.calculate(target, instances);
		int subL = index + 1;
		int subR = instances.size() - index - 1;
		double subResL = ((double) subL) / ((double) totalN) * 
				(1 - Gini_Impurity.calculateConti(target, instances, 0, index));
		double subResR = ((double) subR) / ((double) totalN) * 
				(1 - Gini_Impurity.calculateConti(target, instances, index + 1, totalN - 1));
		//double subResR =  (1 - Gini_Impurity.calculateConti(target, instances, index + 1, totalN - 1));
		gini_imp -= (subResL + subResR);
		return gini_imp;
	}
